...
public class GrottoSearchManager {
    ...
    public static synchronized void toggleSearch() {
        if (loopActive) {
            clearSearch();
            endTasks();
            ChatUtils.addMessage("§2[SBC§r§d-GS§r§2] §r§cSearch stopped");
        }
        else {
           	clearSearch();
            ChatUtils.addMessage("§2[SBC§r§d-GS§r§2] §r§aSearch started");
            runSearchLoop();
        }
    }
    ...
    public static synchronized void clearSearch() {
    	if (found) {
            found = false;
        	ChatUtils.addMessage("§2[SBC§r§d-GS§r§2] §r§eSearch cleared");
    ...
    public static synchronized void scan() {
    	if (active) {
			active = false;
			ChatUtils.addMessage("§2[SBC§r§d-GS§r§2] §r§cScan cancelled");
		    endTasks();
		    return;
		}
    	ChatUtils.addMessage("§2[SBC§r§d-GS§r§2] §r§aScan started");
    ...
    public static void listSearch() {
    	if (foundBlocks.isEmpty()) {
			ChatUtils.addMessage("§2[SBC§r§d-GS§r§2] §r§cNo blocks found");
		}
    	else {
    ...
    private static void runScanTaskAsync(int timeout, Runnable onFound, Runnable onTimeout, boolean single) {
        if (active) {
            ChatUtils.addMessage("§2[SBC§r§d-GS§r§2] §r§cA scan is already in progress!");
            return;
        }
        active = true;
            ...
            while (!Thread.currentThread().isInterrupted() && !foundInTask.get()
                ...
                glassScanTask = new BlockScanTask(
            		pos -> {
	                    ...
	                    ChatUtils.addMessage("§2[SBC§r§d-GS§r§2] §r§aFound Magenta Stained Glass §r" + roundedDistance + " blocks away");
	                    String block = pos.getX() + " " + pos.getY() + " " + pos.getZ();
	                    ChatUtils.sendFormattedMessage(
        				    Text.literal("§b" + block).setStyle(Style.EMPTY
        				        .withClickEvent(new ClickEvent.CopyToClipboard(block))
                ...
                paneScanTask = new BlockScanTask(
            		pos -> {
	                    ...
	                    double distance = Math.sqrt(Math.pow(client.player.getX() - pos.getX(), 2)
	                            ...
	                            + Math.pow(client.player.getZ() - pos.getZ(), 2));
	                    double roundedDistance = Math.round(distance * 10.0) / 10.0;
	                    ChatUtils.addMessage("§2[SBC§r§d-GS§r§2] §r§aFound Magenta Stained Glass Pane §r" + roundedDistance + " blocks away");
	                    String block = pos.getX() + " " + pos.getY() + " " + pos.getZ();
	                    ChatUtils.sendFormattedMessage(
        				    Text.literal("§b" + block).setStyle(Style.EMPTY
        				        .withClickEvent(new ClickEvent.CopyToClipboard(block))
        		...
        		if (firstRun && !foundInTask.get()) {
        			firstRun = false;
    				ChatUtils.addMessage("§2[SBC§r§d-GS§r§2] §r" + (single ? "§cNo blocks found" : "§cNo blocks found. §r§eLooping..."));
        			if (single) {
						active = false;
						break;
        			}
    ...
    private static synchronized void setFoundBlock(BlockPos pos) {
        ...
        ArrayList<Vec3d> coords = new ArrayList<>();
        coords.add(new Vec3d(pos.getX(), pos.getY(), pos.getZ()));
